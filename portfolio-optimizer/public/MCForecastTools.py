...
class MCSimulation:
    ...
    def __init__(self, portfolio_data, weights="", num_simulation=1000, num_trading_days=252):
        ...
        if not "daily_return" in portfolio_data.columns.get_level_values(1).unique():
            ...
            portfolio_data = portfolio_data.merge(
                close_df, left_index=True, right_index=True).reindex(columns=tickers, level=0)
    ...
    def calc_cumulative_return(self):
        ...
        last_prices = self.portfolio_data.xs(
            'close', level=1, axis=1)[-1:].values.tolist()[0]
        ...
        for n in range(self.nSim):
            ...
            for s in range(len(last_prices)):
                ...
                for i in range(self.nTrading):
                    ...
                    simvals[s].append(
                        simvals[s][-1] * (1 + np.random.normal(mean_returns[s], std_returns[s])))
        ...
        self.confidence_interval = portfolio_cumulative_returns.iloc[-1, :].quantile(q=[
                                                                                     0.025, 0.975])
