...
def uniswap_lookup(portfolio_imput):
    ...
    x=0
    while x < len(df_dict):
        line_plot_dict[str(found_history[x])] = mc_dict[str(found_history[x])].plot_simulation()
        line_plot_dict[str(found_history[x])].figure.savefig(f'{str(found_history[x])}_line_plot')
        x += 1
    ...
    while x < len(mc_dict): 
        ...
        simulated_returns_data_dict[str(found_history[x])] = pd.DataFrame(simulated_returns_data_dict[str(found_history[x])])
        simulated_returns_data_dict_nums[str(found_history[x])] = simulated_returns_data_dict[str(found_history[x])]
        simulated_returns_data_dict[str(found_history[x])] = simulated_returns_data_dict[str(found_history[x])].plot()
        simulated_returns_data_dict[str(found_history[x])].figure.savefig(f'{str(found_history[x])}_simulated_returns')   
        x += 1
    ...
    sns.heatmap(correlation, vmin=-1, vmax=1).figure.savefig('portfolio_correlation')
